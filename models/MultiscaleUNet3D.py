...
class MultiscaleUNet3D(nn.Module):
    ...
    def forward(self, x):
        ...
        out_layer.shape :  torch.Size([1, 5, 144, 192, 192])
        """
...
class MultiscaleUNet3D(nn.Module):
    ...
    def __init__(self, input_data=4, output_data=5, degree=16):
        ...
        self.mid_layer = SingleConvBlock(kn[1], kn[2])
        self.up_unit1 = UpBlock(kn[2], kn[1])
        self.up_unit2 = UpBlock(kn[1], kn[0])
    ...
    def forward(self, x):
        ...
        mid = self.mid_layer(x1)
        print('mid_layer.shape : ', mid.shape)

        x = self.up_unit1(mid, x1)
        print('up_unit1.shape : ', x.shape)
